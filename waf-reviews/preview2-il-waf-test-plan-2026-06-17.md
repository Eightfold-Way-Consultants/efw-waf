...
2. **Windows fires — DONE ✓ (2026-06-17):** `AWS-Windows` COUNTs Windows command-**injection** but (correctly) ignores bare tool names. Query-arg: `& whoami` → `AWS-Windows` COUNT; `cmd.exe /c dir`, `powershell -enc`, `%SystemRoot%` → NO match (CountedRequests=1) — desirable (blocking any text with "powershell" = FP nightmare; it flags operator+command injection). Body (`/tw` POST, see below): `& whoami` → `AWS-Windows` COUNT. So the rule fires on real injection in both query args and body. (`certutil` cradle probe blocked by local sandbox, not WAF.)
3. **No NEW false positives — DONE ✓ (2026-06-17):** legit "scary" free text in query args AND in a real same-origin POST body (`/tw` feedback) — `O'Brien`, `earnings < $2000 & rent > $800`, `select a plan or drop coverage union`, unicode — matched **ZERO** rules (`AWS-SQLi`/`AWS-Windows`/`AWS-AdminProtection`/`CommonRuleSet`). No FP from the new groups.
   - **`_BODY` coverage confirmed (settled the in-session question):** direct `/tw` POST (origin 403 on bogus reCAPTCHA token → no ticket; WAF inspects at edge first) — injection body → `AWS-Windows`(`& whoami`)+`AWS-SQLi`(`1' OR`/`union select`) COUNT; FP body → clean. An earlier in-session estimator nickname-injection showed no match — confirmed a **test artifact** (nickname inert for anon users), not a WAF gap. Caveat: WAF inspects only ~first 8KB of body → a max-household `query.aspx` postback could exceed it (TODO: max-household body-size check; see `dynamic-paths-inventory-2026-06-17.md`).
   - **Feedback form is reCAPTCHA-gated (finding):** the homepage feedback form (`a.social-button.feedback` → `.comment-form-3` modal → POST `/tw/tasklists/{list}/tasks.json`) is reCAPTCHA **v2**; a headless browser gets an **image challenge** → no token → client blocks the POST (no `/tw` fired, no ticket). Real bot-resistance — the Turnstile migration must preserve equivalent. Dynamic-path inventory: `dynamic-paths-inventory-2026-06-17.md`.
