...
5. **TODO — purge the fake accounts already created.** The hostile `Register→200`s wrote live junk users into the logon DB (260616: ~17 created; the *delivered* ones carry real third-party victim addresses — e.g. `mark.hage@ncsc.nl` (NL national cyber-security centre), `k.jiles@uhaul.com`, dot-mangled Gmails — used for **confirmation-email bombing of third parties**, which also poisons our SES bounce rate). **Mitigation tasks:** (a) identify unconfirmed accounts created from Tor/OVH/datacenter IPs in the abuse window (filter `Created` timestamp + never-`EmailConfirmed`); (b) disable/delete them; (c) consider purging the matching SES suppression entries only after the endpoint is protected; (d) decide whether to notify high-value victims (e.g. NCSC) — owner's call, do not act outward without authorization. Bounced addresses are on the SES suppression list (us-west-2), retrievable via `sesv2 list-suppressed-destinations`.
