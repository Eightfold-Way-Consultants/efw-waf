...
- [ ] **Client-IP handling behind CloudFront (XFF + IPv6) — consolidated.** After cutover the origin's TCP source becomes a CloudFront *edge* IP (varies per request), so every component that reads client IP must use `X-Forwarded-For` / `CloudFront-Viewer-Address` and be IPv6-safe. CloudFront is dual-stack → IPv6-heavy. Three sites, same root cause:
  1. **Estimator session IP-pinning** (`bp101-interface/session/EngineSession.cs` `CurrentIP()`/`IsValidIPRequest()`; trigger at `session.ui/ScreenPage.cs:150` → bounce to picker). Reads XFF-first, so NOT a clobber-everyone blocker (preview2 tested clean), BUT: (a) **crash** — IPv6-created session + later IPv4 request → `CreatorIP.Substring(0,-1)` ArgumentOutOfRangeException → 500 mid-walk (dual-stack/mobile network flip); (b) IPv6-current request disables the check entirely; (c) `/24` heuristic is IPv4-only; (d) takes the **first** XFF token (client-spoofable — CloudFront appends the real viewer as the **last** token, doesn't strip client XFF). Fix: prefer `CloudFront-Viewer-Address` / last XFF token; version-aware compare (IPv6 → /64 or full, never `Substring(0,-1)`). **FIX, do NOT retire — the echo phenomenon is LIVE (measured 2026-06-18):** web-06 logs show ~24 "foreign IP address rejected" bounces in ~2.4 days (~10/day), incl. Microsoft Office365 Safe-Links IPs (`104.47.64/65.254`) re-fetching emailed `?u=` URLs and Tencent/Fastly scanners — the modern Trend-Micro echo the pin guards against. Retiring re-exposes sessions to clobbering (bad in the MN DHS world). **BUT the pin also false-positives on real users (proven 2026-06-18):** an `az.db101.org` user created a session on T-Mobile, reached RESULTS, switched to home wifi (mobile→wifi handoff), then got hard-wedged out of the `gconf` what-if planner (7 retries → dead session) because the `/24` IP-match treated their new network as foreign. So the fix must **REDUCE false positives, not just be IPv6-safe**: the true echo signature is *same URL from 2 IPs within seconds* (not any IP delta) — reject on that, lean on the existing `HasSeenUrl`/session continuity, and post-CloudFront use the WAF Challenge token as the real-browser signal rather than raw IP; if keeping IP, compare against the SET of session IPs not just CreatorIP. See memory `estimator-ip-pinning-cloudfront`. **Gates public cutover** (mobile protocol-flip → both intermittent 500s AND legit-user lockouts; live echo defense must survive CloudFront without wedging mobile users).
...
- [x] **XFF logging — web-04 DONE 2026-06-19.** Added `X-Forwarded-For` custom W3C field + `Referer`/`TimeTaken`/`BytesSent` flags at `siteDefaults` (inherited by all 71 sites; verified preview2-ca picks it up). web-04 logs at `C:\Windows\system32\LogFiles\W3SVC<id>\` (NOT the default inetpub path). Appears in new log files at next rollover.
  - **web-06 SCHEDULED for 2026-06-20 01:00 PT (08:00 UTC)** via a one-time SYSTEM scheduled task (`EFW-XFF-Logging-OneTime` → `C:\temp\add-xff-logging.ps1`, self-logging + self-deleting). Deliberately off-hours because **every `applicationHost.config` edit recycles the app pools** (`disallowRotationOnConfigChange=false` on all pools) and **`BPSession` is in-proc → a recycle wipes all live estimator sessions.** web-06 already has Referer/TimeTaken/BytesSent, so the task only adds the XFF custom field. web-06 logs at `%SystemDrive%\inetpub\logs\LogFiles`.
  - ⚠️ **The recycle-drops-InProc-sessions constraint applies to ALL web-06 `applicationHost.config` changes** (CSP headers, Phase-5 `X-Origin-Verify`) and to code deploys (new `bin\` DLLs restart the app domain). Batch them into low-traffic windows / fold into the cutover deploy so public users aren't repeatedly wiped. See memory `iis-confchange-recycles-inproc`.
...
- [x] **Print hairpin cleared by CODE, not a hosts-pin (supersedes the old "hosts-file pin both servers" item).** Two fixes, both applied+validated on s4/preview2: (1) `pdf.js --host-resolver-rules=MAP *.zone 127.0.0.1` remaps the Puppeteer/Chrome render fetches to loopback (chosen 2026-06-15 over a system hosts-file pin — scoped to the render Chrome, no whole-box DNS override); (2) the .NET self-calls (PDFJob spec, ReportTOC SOAP, `PDFReportSpec`) connect loopback + present origin `Host` (Chrome's resolver doesn't cover .NET). **web-06: deliver BOTH via the code/pdfnode deploy (item under Application changes / Phase 3 gate) — NOT a hosts-pin.** System hosts-file pin remains only a documented fallback if host-resolver-rules ever fails. See memory `pdfreport-print-pipeline`.
...
- [x] **`planning-generic.eightfoldway.com` — RESOLVED 2026-06-19.** It's a WebDeploy scaffold site on web-04 (`C:\inetpub\wwwroot\planning-generic`), not a content site — the real issue was it was **Started and publicly bound on :443**, serving the generic planning app off the open origin (403 at root). pubxml proved WebDeploy targets it **by site NAME** (`DeployIisAppPath=planning-generic`) via WMSVC to `preview2-site`/`web-6b`:8172 — the `planning-generic.eightfoldway.com` host + `:443` binding were used by **nothing**. Actions: **Stopped the site + `serverAutoStart=false`; removed the web-04 `:443` binding; deleted the `planning-generic.eightfoldway.com` CNAME** (was → `edit-site.eightfoldway.com`; backup `waf-reviews/dns-audit/planning-generic-deleted-2026-06-19.json`). **Kept** the site object (WebDeploy targets by name) + wwwroot (read at runtime by `GenericScreenPath.cs`). Host now NXDOMAINs; even if hit, the catch-all 404s it. **web-06 twin:** `planning-generic` site exists there too (public-site.pubxml → web-6b), already **Stopped**, binding `*:80:planning-generic` (bare host) — left as-is; removing its binding would recycle public InProc sessions, so defer to a low-traffic window (it's already Stopped, not serving). Reminder: binding changes edit `applicationHost.config` → recycle pools (fine on web-04; window-only on web-06).
- [x] **Athena tables over the WAF/CloudFront log bucket — DONE 2026-06-19** as IaC: `cloudformation/diagnostics.yaml` (Glue db `efw_waf_logs`, `waf_preview2`/`waf_public` + `cf_preview2`/`cf_public` tables with date partition projection, workgroup `efw-diagnostics`, saved diagnostic queries). Deployed us-east-1; test query verified (preview2: ALLOW 7349 / CHALLENGE 33 / BLOCK 4 since 06/17). Public tables empty until Phase 3/4. DDL source: `waf-reviews/test-diagnostics-plan-2026-06-10.md` §2.
...
- [ ] Verify the print path (pdf.js host-resolver-rules + .NET loopback self-calls): generate a PDF on a preview2 site with Block live — render must go via loopback, untouched by the Challenge (no system hosts-pin involved)
...
**Gate:** web-06 `s6.eightfoldway.com` 443 binding in place (Phase -1); print-path fix (pdf.js host-resolver-rules + .NET loopback PDF/Estimator code) deployed+verified on web-06; origin cert validity through the Count window; **estimator IP-pin fix (svn r8968 — XFF-last-token + IPv6-safe `_SameHost` + established-IP migration allow) deployed to web-06** (proven on preview2; gates public so a mobile↔wifi user isn't wedged and an IPv6 flip doesn't 500).
