...
- [ ] **General stray-Host handling (revised 2026-06-15):** decided NOT to add a no-host-header catch-all IIS site. Subdomain-takeover is already prevented by CloudFront wildcard aliases + the ACM cert (a stray `*.zone` name can only be answered by our distribution). The dedicated `s4`/`s6` 404 sites cover literal hits on the origin FQDNs. For an arbitrary unmatched Host arriving at origin via CloudFront (would otherwise 502), handle it at CloudFront with a **Custom Error Response** (502/504 → branded 404 page) — one place, no per-server config. (Open: add the custom-error config to the edge stacks.)
